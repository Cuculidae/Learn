/*
 * The IndexMaxPQ class represents an indexed maximum-oriented priority queue of generic keys.
 ...
 * The insert, delete-the-maximum, delete, operations take logarithmic time.
 * The is-empty, size, max-index, max-key, and key-of operations take constant time.
 * Construction takes time proportional to the specified capacity.
 * For additional documentation, see <href="http://algs4.cs.princeton.edu/24pq">
 *
 * Execution:		java IndexMaxPQ < input
...
public class IndexMaxPQ<Key extends Comparable<Key>> implements Iterable<Key>
{
     ...
     */
	IndexMaxPQ(IndexMaxPQ<Key> that)
	{
		N = that.N;
		pq = new int[N+1];
		qp = new int[N+1];
	...
	public int delMax()
	{
		...
		int indexOfMax = pq[1];
		qp[indexOfMax] = -1;
		keys[indexOfMax] = null; // avoid loitering
		pq[1] = pq[N--];
		sink(1);
		return indexOfMax;
	}
    ...
    private class HeapIterator implements Iterator<Key>
    {
        ...
        public HeapIterator(IndexMaxPQ<Key> that) { copy = new IndexMaxPQ<Key>(that); }
	 ...
	 *********************************************************************************************/
	//Unit-test IndexMaxPQ data type
	public static void main(String[] args)
	{ 
		String[] a = In.readStrings();
		IndexMaxPQ<String> pq = new IndexMaxPQ<String>(a.length);
